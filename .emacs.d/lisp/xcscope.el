; http://sourceforge.net/tracker/index.php?func=detail&aid=1839942&group_id=4664&atid=304664
...
;; gotten into current position. I used to ask myself ??ow I get into
;; function???since the code I?? reading has really deep call stacks and I
;; can?? keep them all in my mind. The other benefit is that, for large
;; projects, cscope-ing sometimes could be time-consuming.

;; USAGE:
;; To view previous search result, call cscope-history-backward (??-c s v??;; or just ????in cscope buffer) or cscope-history-forward (????.
;; To discard cscope history, M-x cscope-history-clear
;; To delete history entry currently visiting, M-x cscope-history-delete

;; fang.lungang at gmail.com
...
(if cscope-list-entry-keymap
  ...
  (define-key cscope-list-entry-keymap (kbd "RET") 'cscope-select-entry-one-window)
...
(defun cscope-update-database-toggle ()
  (interactive)
  (setq cscope-do-not-update-database
	(not cscope-do-not-update-database)))
